...
//        }
    ...
    public void setCellOutOfBounds1() {
        GameField gameField = new GameField();
        TestCaseCell testCaseCell = new TestCaseCell('r', 4, CellState.MISSED);
        gameField.getCellState(testCaseCell.ch, testCaseCell.num);
    }

    @Test(expected = IndexOutOfBoundsException.class)
    public void setCellOutOfBounds2() {
        GameField gameField = new GameField();
        TestCaseCell testCaseCell = new TestCaseCell('h', 15, CellState.SHIP);
        gameField.getCellState(testCaseCell.ch, testCaseCell.num);
    }

    @Test(expected = IndexOutOfBoundsException.class)
    public void setCellOutOfBounds3() {
        GameField gameField = new GameField();
        TestCaseCell testCaseCell = new TestCaseCell('k', 11, CellState.MISSED);
        gameField.getCellState(testCaseCell.ch, testCaseCell.num);
    }
    ...
    public void checkShipDestroyedTest() {
        ...
        for (TestCase tc : testCases) {
            ...
            for (CellCoordinates cc : tc.shipHit) {
                assertEquals(tc.shipIntact.isEmpty(), gameField.checkShipDestroyed(cc));
            }
            for (CellCoordinates cc : tc.shipIntact) {
                assertEquals(tc.shipIntact.isEmpty(), gameField.checkShipDestroyed(cc));
            }
